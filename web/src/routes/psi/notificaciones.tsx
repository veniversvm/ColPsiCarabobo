...
import { createMemo, createResource, createSignal, For, Show, Suspense, onMount } from "solid-js";
...
// El panel psi es privado y autenticado (sin beneficio de SEO): esta página se
// renderiza SOLO en el cliente (mismo patrón que el kanban de proyectos), con
// skeleton en SSR. Evita de raíz mismatches de hidratación (p. ej. fechas
// calculadas con la timezone del server vs la del navegador) y simplifica el
// ciclo recursos→fetch (todo vive en sessionStorage).
export default function PsiNotificaciones() {
  return (
    <Show when={!isServer} fallback={<NotificacionesSkeleton />}>
      <NotificacionesContenido />
    </Show>
  );
}

function NotificacionesContenido() {
  ...
  // Primera página vía resource; las páginas siguientes se cargan con keyset
  // pagination (cursor) sin recargar la ruta.
  const LIST_LIMIT = 20;
    ...
    <main class="bg-colpsi-bg min-h-screen pb-24">
      ...
      <div class="max-w-3xl mx-auto px-4 -mt-12 space-y-3">
        <Suspense fallback={
          ...
          <For each={items()}>
            {(n) => {
              // OJO: <For> (mapArray sin keyed) NO re-ejecuta el callback del item
              // cuando cambian señales; un `const` aquí queda congelado en su primer
              // valor y el pill nunca voltea / el cuerpo nunca expande. Cada derivado
              // reactivo del item DEBE ser un memo.
...
// Shell estático que emite el SSR: sin datos ni hidratación que romper. El
// contenido real (NotificacionesContenido) se monta solo en el navegador.
  ...
  return (
    <main class="bg-colpsi-bg min-h-screen pb-24">
      <div class="bg-heraldic pt-12 pb-20 px-6 shadow-inner">
        <div class="max-w-3xl mx-auto">
          <A href="/psi" class="inline-flex items-center gap-1 text-blue-200 text-sm font-bold mb-4 hover:text-white">← Volver al Panel</A>
          <h1 class="text-white text-2xl font-bold flex items-center gap-3">🔔 Notificaciones</h1>
          <p class="text-blue-200 text-sm mt-1">Comunicados y avisos del colegio</p>
        </div>
      </div>
      <div class="max-w-3xl mx-auto px-4 -mt-12 space-y-3">
...
function formatDate(value?: string | null): string {
  ...
  // timeZone fija: en SSR (Deno/docker corre en UTC) y navegador (America/Caracas)
  // muestra la MISMA fecha; evita días corridos en notificaciones nocturnas.
  return d.toLocaleDateString("es-VE", { year: "numeric", month: "short", day: "numeric", timeZone: "America/Caracas" });
}
