import { createResource, For, Show, Suspense, createSignal, createEffect } from "solid-js";
...
export default function AcademicoPage() {
  ...
  // 2. Estados de Control de UI
  const [showForm, setShowForm] = createSignal(false);
  const [isEditing, setIsEditing] = createSignal<string | null>(null);
  const [loading, setLoading] = createSignal(false);

  // 3. Store del Formulario (Nombres simples para el DTO de Go)
  ...
  const openEdit = (pg: any) => {
    setIsEditing(pg.id);
    // MAPEAMOS: Del nombre de la DB (post_grade_...) al nombre del Formulario
    setForm({
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar este título de forma permanente?")) return;
    try {
  ...
  return (
    <main class="bg-[#f8fafc] min-h-screen pb-24 font-sans">
      ...
      <div class="max-w-4xl mx-auto px-4 md:px-8 -mt-10 space-y-8">
          ...
          <form onSubmit={handleSubmit} class="bg-white rounded-[2.5rem] p-8 shadow-2xl border-2 border-colpsi-yellow animate-in slide-in-from-top-4 duration-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-1">
                ...
                <input type="text" required value={form.title} onInput={(e) => setForm("title", e.currentTarget.value)} class="w-full bg-gray-50 border-2 border-transparent focus:border-colpsi-yellow rounded-xl px-5 py-3 outline-none" />
              </div>
              <div class="space-y-1">
                <label class="text-xs font-black text-gray-400 uppercase ml-2">Universidad</label>
                <input type="text" required value={form.university} onInput={(e) => setForm("university", e.currentTarget.value)} class="w-full bg-gray-50 border-2 border-transparent focus:border-colpsi-yellow rounded-xl px-5 py-3 outline-none" />
              </div>
              <div class="space-y-1">
                <label class="text-xs font-black text-gray-400 uppercase ml-2">Año de Egreso</label>
                <input type="number" required value={form.graduation_year} onInput={(e) => setForm("graduation_year", e.currentTarget.value)} class="w-full bg-gray-50 border-2 border-transparent focus:border-colpsi-yellow rounded-xl px-5 py-3 outline-none" />
              </div>
              
              <div class="md:col-span-2 space-y-1">
                <label class="text-xs font-black text-gray-400 uppercase ml-2">Breve descripción</label>
                <textarea value={form.description} onInput={(e) => setForm("description", e.currentTarget.value)} class="w-full bg-gray-50 border-2 border-transparent focus:border-colpsi-yellow rounded-xl px-5 py-3 outline-none" />
              </div>
              ...
              <div class="md:col-span-2 pt-4">
                <button type="submit" disabled={loading()} class="w-full bg-colpsi-blue text-white py-4 rounded-2xl font-black shadow-lg">
        ...
        {/* LISTADO DE POSTGRADOS (MAPEADO SEGÚN TU JSON) */}
              ...
              {(pg) => (
                <div class="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-premium border border-gray-100 flex flex-col md:flex-row gap-6 relative group overflow-hidden">
                  ...
                  <div class="absolute top-6 right-6 flex gap-2">
                    <button onClick={() => openEdit(pg)} class="w-10 h-10 bg-blue-50 text-colpsi-blue rounded-xl flex items-center justify-center hover:bg-colpsi-yellow transition-colors">✏️</button>
                    <button onClick={() => handleDelete(pg.id)} class="w-10 h-10 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors">🗑️</button>
                  </div>
                  ...
                  <div class="flex-grow space-y-3">
                    ...
                    {/* Miniaturas de Soportes */}
                    <div class="flex gap-3 pt-4">
                      <For each={[pg.pic_one_url, pg.pic_two_url, pg.pic_three_url]}>
                        {(url) => (
                          <Show when={url}>
                            <a href={`http://localhost:9000/colpsi-bucket/${url}`} target="_blank" class="w-16 h-20 bg-gray-50 rounded-xl border border-gray-200 overflow-hidden hover:ring-2 hover:ring-colpsi-yellow transition-all">
