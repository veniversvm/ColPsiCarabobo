  ...
  return (
    <div class="flex flex-col xl:flex-row gap-10">
        ...
        <div class="flex flex-col items-center space-y-4">
          <label class="text-[11px] font-semibold text-colpsi-muted uppercase tracking-wide">
            Foto de Perfil
          </label>
          <div class="relative group">
            <div class="w-32 h-32 rounded-lg overflow-hidden border-4 border-gray-50 shadow-md bg-gray-100 flex items-center justify-center relative">
              <Show 
                when={previewUrl()} 
              ...
              {/* Overlay de carga */}
              <label class="absolute inset-0 bg-colpsi-blue/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center cursor-pointer text-white p-2 text-center">
          ...
          <p class="text-[9px] text-colpsi-muted font-medium max-w-[120px] text-center">
            Formatos: JPG, PNG. Máx 2MB.
          </p>
          <Show when={previewUrl() && !props.avatarFile}>
            <button
              onClick={() => props.onDeletePicture()}
              class="text-[11px] font-semibold uppercase tracking-wide bg-red-50 text-red-600 border border-red-200 rounded-lg px-3 py-1.5 hover:bg-red-100 transition-colors"
        ...
        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div class="md:col-span-2 p-6 bg-blue-50/50 rounded-lg border border-blue-100 mt-2">
            <p class="text-sm text-blue-700 leading-relaxed">
              <span class="font-semibold uppercase mr-1">Aviso:</span>
        ...
        <div class="flex flex-col items-center justify-start lg:border-l lg:border-colpsi-border lg:pl-8">
          <label class="text-[11px] font-semibold text-colpsi-muted uppercase tracking-wide mb-4">
            Acceso Directo (QR)
          </label>
          <QRCodeGenerator url={props.url} />
          <p class="text-[9px] text-colpsi-muted mt-4 text-center max-w-[150px]">
